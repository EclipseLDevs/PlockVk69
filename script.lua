if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("VKontakte GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "VKontakte") then
				v:Destroy()
			end
		end
	end
...
local T1UIColor = {
    ...
    ["Logo Image"] = "rbxassetid://103324203833614",           -- ÍCONE DO BANANA CAT HUB
...
Library_Function.Gui.Name = 'VKontakte GUI'
...
ImageLabel.Image = "rbxassetid://103324203833614"
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "VKontakte") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "VKontakte"
   ...
   TextLabelMain.Text = "<font color=\"rgb(0,0,0)\">" .. tostring(TitleNameMain or "VKontakte") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    return keybindFunction
        ...
        function pagefunc:AddDiscordInvite(setting)
            local sec = pageFunction:AddSection(setting.Name or "Discord")
            pcall(function()
                sec:AddButton({Name = "📎 Entrar: " .. (setting.Name or "Discord"), Title = ""}, function()
                    pcall(function() setclipboard(setting.Invite or "") end)
                end)
            end)
        end
        function pagefunc:AddParagraph(setting)
            ...
            local lbl = nil
            pcall(function() lbl = sec:AddLabel(setting.Desc or "") end)
            local obj = {_sec = sec, _lbl = lbl}
            function obj:SetDesc(text)
                pcall(function()
                    if self._lbl then
                        self._lbl.Text = tostring(text)
                    end
                end)
            end
            return obj
        ...
        function pagefunc:AddTextBox(setting, cb)
            local sec = pageFunction:AddSection(setting.Title or "Input")
            pcall(function() sec:AddInput("input", {Name = setting.Title or "", Callback = cb or function() end}) end)
        end
		return pagefunc
        end
...
-- VKontakte Hub
--[[
    VKontakte Hub - Optimized Version
...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://103324203833614"
    ...
    if not success then
        warn("[VKontakte] Erro ao exibir notificação")
    end
end

NotificacaoNightMystic("VKontakte Hub", "Script carregado com sucesso!")
...
local Status = Window:AddTab("Status And Server", "rbxassetid://7040410130")

local Shop = Window:AddTab("Shop", "rbxassetid://6031265976")

local Farm = Window:AddTab("Tab Farming", "rbxassetid://127561653320876")

local Maestry = Window:AddTab("Tab Farm Maestry", "rbxassetid://10709782497")

local Others = Window:AddTab("Tab Farm Others", "rbxassetid://10723405360")

local Event = Window:AddTab("Tab Sea Event", "rbxassetid://10709783577")

local Race = Window:AddTab("Tab Race Upgrade", "rbxassetid://115164375298022")

local Dojo = Window:AddTab("Tab Dojo Quest & Drago Race", "rbxassetid://91920478152016")

local Esp = Window:AddTab("Tab Stats & ESP", "rbxassetid://11155851001")

local Player = Window:AddTab("Tab Local Player", "rbxassetid://13075651575")

local Teleport = Window:AddTab("Tab Teleport", "rbxassetid://10734886004")

local Get = Window:AddTab("Tab Get Items & Upgrade", "rbxassetid://13075622619")

local Fruit = Window:AddTab("Tab Raid & Fruit", "rbxassetid://11155986081")

local Setting = Window:AddTab("Tab Settings & Misc", "rbxassetid://7734053495")
Discord:AddDiscordInvite({
    Name = "VKontakte Hub",
    Description = "Join for support and updates",
    Logo = "rbxassetid://103324203833614",
    Invite = "https://discord.gg/BUtspHxrj"
})
...
NotificacaoNightMystic("VKontakte Hub", "✅ Todos os sistemas carregados!")
